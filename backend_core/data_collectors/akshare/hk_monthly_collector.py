...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('hk_monthly_generation.log', encoding='utf-8'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class HKMonthlyDataGenerator:
    """港股月线数据生成器"""
    ...
    def _init_db(self):
        ...
        try:
            self.session.execute(text('''
                CREATE TABLE IF NOT EXISTS hk_monthly_quotes (
                    code TEXT,
                    ts_code TEXT,
                    name TEXT,
                    market TEXT,
        ...
        try:
            ...
            for date, row in monthly_df.iterrows():
                try:
                    ...
                    logger.error(f"保存港股 {stock_code} 月线数据失败: {e}")
                    continue
            ...
            logger.error(f"生成港股 {stock_code} 月线数据失败: {e}")
            self.failed_count += 1
            return False
    ...
    def generate_monthly_data(self, start_date: str, end_date: str, stock_codes: Optional[List[str]] = None) -> Dict[str, any]:
        """批量生成港股月线数据"""
        try:
            logger.info(f"开始生成港股月线数据: {start_date} 到 {end_date}")
            ...
            logger.info(f"港股月线数据生成完成: {result}")
            return result
            
        except Exception as e:
            logger.error(f"批量生成港股月线数据失败: {e}")
            return {'total': 0, 'success': 0, 'failed': 1}
    ...
    def _log_operation_result(self, start_date: str, end_date: str, total_stocks: int, success_stocks: int):
        ...
        try:
            ...
            """), {
                'operation_type': 'generate_hk_monthly_from_daily',
...
def main():
    ...
    parser = argparse.ArgumentParser(description='基于日线数据生成港股月线数据')
    ...
    generator = HKMonthlyDataGenerator()
