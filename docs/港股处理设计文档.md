...
**换手率自动补充机制**:
- **功能**: 当查询结果中换手率为 `None` 或 `0` 时，自动从 akshare 接口获取并更新到数据库
- **支持范围**: 同时支持A股和港股
- **实现逻辑**:
  1. 检测查询结果中换手率为空的记录
  2. 根据股票类型选择对应的 akshare 接口:
  ...
  5. 更新查询结果中的换手率字段，立即返回给前端
- **触发条件**: 
  - 查询结果中存在 `turnover_rate` 为 `None` 或 `0` 的记录
  - 提供了 `start_date` 和 `end_date` 参数
  - akshare 库可用
- **日期格式处理**: 
  - akshare 返回的日期格式可能是 `YYYYMMDD` 或 `YYYY-MM-DD`
  - 系统统一转换为 `YYYY-MM-DD` 格式进行匹配
- **错误处理**: 
  - 如果 akshare 查询失败，不影响主查询流程
  - 记录错误日志，继续返回原始查询结果
...
### 6.5 换手率数据补充策略
- **设计原则**: 按需补充，不影响主查询流程
- **补充时机**: 仅在查询时发现换手率为空时触发
- **数据源选择**: 
  - A股和港股使用不同的 akshare 接口
  - 根据 `is_hk_data` 标志自动选择正确的接口
- **批量处理**: 一次查询补充所有缺失日期的换手率，提高效率
- **容错机制**: 
  - akshare 查询失败不影响主查询结果
  - 单个日期更新失败不影响其他日期的更新
  - 记录详细的错误日志便于排查问题
...
- **v1.1** (2025-11-24): 新增港股换手率自动补充功能，修复港股历史行情查询时换手率数据缺失的问题
