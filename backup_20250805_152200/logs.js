...
class LogsManager {
    ...
    bindEvents() {
        ...
        document.getElementById('statusFilter').addEventListener('change', () => this.updateFilters());
        document.getElementById('operationTypeFilter').addEventListener('input', () => this.updateFilters());
        document.getElementById('stockCodeInput').addEventListener('input', () => this.updateFilters());
    }
...
function initLogsManager() {
    ...
    if (!logsPage) {
        ...
        // 将延迟从200ms增加到500ms，给DOM更多时间渲染
        setTimeout(initLogsManager, 500); // 增加延迟时间
        return;
    }
