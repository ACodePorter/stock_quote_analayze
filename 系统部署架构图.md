...
graph TB
    ...
    subgraph "应用服务器 192.168.16.4"
        subgraph "前端应用"
            Frontend[前端应用<br/>端口: 8000<br/>目录: run/frontend<br/>启动: python start_frontend.py]
        end
        
        subgraph "管理后台"
            AdminModern[管理后台<br/>端口: 8001<br/>目录: run/admin-modern<br/>启动: python -m http.server 8001]
        end
        
        subgraph "后端API"
            BackendAPI[后端API<br/>端口: 5000<br/>目录: run/backend_api<br/>启动: python run.py]
        end
        
        subgraph "数据采集"
            BackendCore[数据采集<br/>目录: run/backend_core<br/>无端口]
        end
    end
    
    subgraph "数据库服务器"
        PostgreSQL[(PostgreSQL<br/>192.168.16.4<br/>数据库: stock_analysis<br/>密码: qidianspacetime$91)]
    end
